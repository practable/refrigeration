...
void adamController::check_modbus_connect() {
  if (!modbusTCP.connected()) {
    ...
    Serial.print(F("{\"error\":\""));    //JSON error message header
    Serial.print(moduleName);
...
int16_t adamController::set_coil(int coilNum, bool coilState) {
  ...
  if ((coilNum >= 0) && (coilNum < 8)) {
    if (!modbusTCP.coilWrite(d_out[coilNum], state)) {
      ...
      sprintf(buffer, "{\"error\":\"%s: Setting Coil: %i ( %#0x ) to %i\"}", moduleName, coilNum, d_out[coilNum], coilState);
    }
  } else {
...
adamController::dataArray adamController::read_analog_inputs() {
  ...
  if (response > 0) {
    ...
    sprintf(buffer, "{\"status\":\"%s: Read Analog Inputs (DAC): %u, %u, %u, %u, %u, %u\"}", moduleName, readBuffer[0], readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4], readBuffer[5]);
  } else {
