...
void build_json() {
  ...
  dtostrf(ps_vals[NUM_PRESSURE_SENSORS], 6, 2, float_buffer);
  sprintf(json_buffer, "%s%s      \"PS%i\" : %6s", json_buffer, newline, NUM_PRESSURE_SENSORS+1, float_buffer);
  ...
  dtostrf(ts_vals[NUM_TEMP_SENSORS], 6, 2, float_buffer);
  sprintf(json_buffer, "%s%s      \"TS%i\" : %6s", json_buffer, newline, NUM_TEMP_SENSORS+1, float_buffer);
...
dtostrf(misc_vals[5], 6, 2, float_buffer);
    sprintf(json_buffer, "%s%s      \"%s\" : %6s", json_buffer, newline, misc_names[5], float_buffer);
